...
"misc")
	...
	"log2ram")
		...
		;;
	# "duckdns")
	# 	whiptail --title "DuckDNS Crontab Setup" --msgbox "Next, we will configure the crontab that will update the redirection to our IP of the domains registered in 'www.duckdns.org' every 5 minutes." 10 78
	# 	DOM=$(whiptail --title "DuckDNS Crontab Setup" --inputbox "Enter the comma-separated list of domains registered in DuckDNS::" 8 78  3>&1 1>&2 2>&3)
	# 	TOK=$(whiptail --title "DuckDNS Crontab Setup" --inputbox "Enter the token generated by DuckDNS:" 20 50 3>&1 1>&2 2>&3) 
	# 	sudo sed --in-place "s/DOMAINS=[^ ]*/DOMAINS=${DOM}/; s/DUCKDNS_TOKEN=[^ ]*/DUCKDNS_TOKEN=${TOK}/" ./duck/duck.sh
